...
interface StrategyV1StableVariable:
    def secondaryBalance() -> uint256: view
    def depositSecondary(): nonpayable   
    def withdrawSecondary(_assets: uint256): nonpayable
...
NAME: constant(String[10]) = "LendVlt"
SYMBOL: constant(String[5]) = "flVlt"
...
def _totalAssets() -> uint256:
    return self.Strategy.secondaryBalance()
...
def previewDeposit(assets: uint256) -> uint256:
    # @Note: Take into account swap fees to return accurate deposit amount
...
def deposit(assets: uint256, receiver: address=msg.sender) -> uint256:
    ...
    self.Strategy.depositSecondary()
...
def withdraw(assets: uint256, receiver: address=msg.sender, owner: address=msg.sender) -> uint256:
    ...
    self.Strategy.withdrawSecondary(assets)
...
def redeem(shares: uint256, receiver: address=msg.sender, owner: address=msg.sender) -> uint256:
    ...
    self.Iasset.transfer(receiver, assets)
    log Withdraw(msg.sender, receiver, owner, assets_withdraw, shares)
    return assets
